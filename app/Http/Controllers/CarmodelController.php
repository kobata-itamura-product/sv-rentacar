...
use App\Models\Carmodel; // Productモデルを現在のファイルで使用できるようにするための宣言です。
...
class CarmodelController extends Controller //コントローラークラスを継承します（コントローラーの機能が使えるようになります）
{
    ...
    public function index()
    {
        ...
        return view('index', compact('car_model'));
    ...
    public function create(Carmodel $car_model)
    {
        // 商品作成画面で会社の情報が必要なので、全ての会社の情報を取得します。
        $car_model = Carmodel::all();
        //dd($car_model);
        // 商品作成画面を表示します。その際に、先ほど取得した全ての会社情報を画面に渡します。
        return view('create', compact('car_model'));
    }
    ...
    //(Product $product) 指定されたIDで商品をデータベースから自動的に検索し、その結果を $product に割り当てます。
    {
        ...
        return view('show', ['car_model' => $car_model]);
